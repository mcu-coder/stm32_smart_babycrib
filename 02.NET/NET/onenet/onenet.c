...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"light\":%d,", 0);
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"rain\":%d", 0);
	strcat(buf, text);
...
unsigned char MQTT_FillBuf(char *buf)
{
	...
	// 有效数据
	memset(text, 0, sizeof(text));
	sprintf(text, "\"temp\":\"%.1f\",", Data_init.temperatuer);
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "\"humi\":\"%.1f\",", Data_init.humiditr);
	strcat(buf, text);
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"warning\":\"%d\",", device_state_init.waning_state);
	strcat(buf, text);

	memset(text, 0, sizeof(text));
	sprintf(text, "\"music\":\"%d\",", device_state_init.music_state);
	strcat(buf, text);

	memset(text, 0, sizeof(text));
	sprintf(text, "\"music_s\":\"%d\",", Data_init.music_s);
	strcat(buf, text);
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"bed\":\"%d\",", device_state_init.bed_state);
	strcat(buf, text);
	
	memset(text, 0, sizeof(text));
	sprintf(text, "\"eat\":\"%d\"", (u8)LEVEL1);
	strcat(buf, text);
...
unsigned char Mqtt_Flage_Info(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"check_device\":\"%d\"", 1); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
