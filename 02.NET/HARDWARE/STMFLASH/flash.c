...
void W_Test(void)
{
    ...
    System_WriteInteriorFlash(addrShort1, (uint8_t *)&threshold_value_init.PH_value, sizeof(threshold_value_init.PH_value)); // 存入数据，函数内自动解锁，擦除，写入
...
void R_Test(void)
{
    ...
    System_ReadInteriorFlash(addrShort1, (uint8_t *)&threshold_value_init.PH_value, sizeof(threshold_value_init.PH_value)); // 读取数据，函数内自动按半字读取
    //printf("1地址:0x%X 读取:%.6f \r\n", addrShort1, longitude_sum);
    ...
    //printf("2地址:0x%X 读取:%.6f \r\n", addrShort2, latitude_sum);
}
/*********************************************************************************
